...
def make_pu_layer_legend_category(setup_object):
    ...
    elif setup_object.analysis_type == 'MarxanWithZones' or setup_object.analysis_type == 'PrioritizrWithZones':
...
def display_distribution_maps(setup_object, dist_shape_file_path_name, abund_values_dict, legend_type, selected_feat_id_list):
    ...
    for featID in selected_feat_id_list:
        ...
        project.addMapLayer(a_dist_layer, addToLegend=False)
        project.layerTreeRoot().insertLayer(0, a_dist_layer)
...
def display_best_output(setup_object, best_field_name, best_shapefile_name):
    ...
    project = QgsProject.instance()
    project.addMapLayer(best_layer, addToLegend=False)
    project.layerTreeRoot().insertLayer(0, best_layer)
