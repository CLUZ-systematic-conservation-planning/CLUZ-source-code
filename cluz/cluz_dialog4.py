...
class IdentifySelectedDialog(QDialog, Ui_identifySelectedDialog):
    def __init__(self, iface, setup_object):
        ...
        selected_pu_count_string = str(len(selected_pu_id_dict))
        selected_pu_details_dict = return_selected_pu_id_details_dict(setup_object, selected_pu_id_dict)
        self.show_selected_identify_data(setup_object, selected_pu_details_dict, selected_pu_count_string)

    def show_selected_identify_data(self, setup_object, selected_pu_details_dict, selected_pu_count_string):
        if len(selected_pu_details_dict) > 0:
            ...
            self.setWindowTitle('Details of ' + selected_pu_count_string + ' planning units.')
        else:
